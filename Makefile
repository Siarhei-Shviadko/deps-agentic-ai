...
APP_NAME = api
NO_DEV_DOCKER_IMAGE = agentic-ai
DEV_DOCKER_IMAGE = agentic-ai-dev
...
build-prod:
	$(call build_service,agentic-ai-dev,./etc/agentic-ai/Dockerfile,,develop)
	$(call build_service,agentic-ai,./etc/agentic-ai/Dockerfile,,,agentic-ai-dev)
	$(call build_service,agentic-ai-migrator,./etc/migrator/Dockerfile)

.PHONY: push
## Push images to registry
push:
	$(call push_service,agentic-ai)
	$(call push_service,agentic-ai-dev)
	$(call push_service,agentic-ai-migrator)
...
tag:
	$(call tag_service,agentic-ai)
	$(call tag_service,agentic-ai-dev)
	$(call tag_service,agentic-ai-migrator)

.PHONY: pull
## Pull service images from docker registry
pull:
	$(call pull_service,agentic-ai)
	$(call pull_service,agentic-ai-dev)
	$(call pull_service,agentic-ai-migrator)
...
helm-upgrade-service:
	helm upgrade --install $(CI_PROJECT_NAME) .helm/services \
        ...
        --set agentic_ai.image.tag=$(CI_COMMIT_SHORT_SHA) \
        --set agentic_ai_consumer.image.tag=$(CI_COMMIT_SHORT_SHA) \
        --set agentic_ai_settings.DATABASE_REQUIRE_SECURE_TRANSPORT=$(DATABASE_REQUIRE_SECURE_TRANSPORT) \
        --set agentic_ai_settings.EXTERNAL_URL=$(SERVICE_EXTERNAL_URL) \
...
.PHONY: build-no-dev
build-no-dev:
	$(call build_service,$(NO_DEV_DOCKER_IMAGE),./etc/agentic-ai/Dockerfile,,build,$(DEV_DOCKER_IMAGE))
