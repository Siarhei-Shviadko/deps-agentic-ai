...
from typing import Any, Collection, Protocol, TypeVar, get_args

from deps_agentic_ai.domain.exceptions import IllegalArgument
...
class LengthCheck:
    ...
    def is_correct(self, domain_obj: Any, value: Collection, attribute_name: AttributeName) -> None:
        if len(value) > self._max_length:
            raise IllegalArgument(
                f"Attribute {attribute_name.public} for {domain_obj.__class__.__name__} object cannot be more "
                f"than {self._max_length} symbols / elements.",  # noqa: WPS326
            )
        if len(value) < self._min_length:
            raise IllegalArgument(
                f"Attribute {attribute_name.public} for {domain_obj.__class__.__name__} object cannot be less "
                f"than {self._min_length} symbols / elements.",  # noqa: WPS326
            )
